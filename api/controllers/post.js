...
exports.getAllPost = (req, res) => {
    dbConnection.query('SELECT p.*, u.lastname, firstname, imageProfile, role FROM post AS p JOIN users AS u ON (u.id = p.userId) ORDER BY p.createdAt DESC', (err, result) => {
        if (err) return res.status(500).json(err);
